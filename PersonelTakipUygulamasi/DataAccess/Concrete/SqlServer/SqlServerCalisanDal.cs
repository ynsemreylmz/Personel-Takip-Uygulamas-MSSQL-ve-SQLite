	...
	public class SqlServerCalisanDal : ICalisanDal
	{
		...
		public List<Calisan> Listele()
		{
			...
			try
			{
				...
				using (SqlCommand command = new SqlCommand($"SELECT * FROM Calisanlar", SqlServerBaglanti.Connection))
				{
					SqlServerBaglanti.BaglantiAc();//Bağlantı oluşturuluyor
		...
		public List<Calisan> Listele(string kosulCumlesi)
		{
			...
			{
				//Using scopeları içinde tanımlanan nesne scope dışına çıkılır çıkılmaz silinir.
				using (var command = new SqlCommand($"SELECT * FROM Calisanlar {kosulCumlesi}", SqlServerBaglanti.Connection))
				{
		...
		public Calisan Getir(string kosulCumlesi)
		{
			...
			{
				int _sonuc = 0;
				using (var command = new SqlCommand($"SELECT COUNT (*) FROM Calisanlar {kosulCumlesi}", SqlServerBaglanti.Connection))
				{
					SqlServerBaglanti.BaglantiAc();
		...
		public bool Kaydet(Calisan calisan)
		{
			...
			try
			{
				using (var command = new SqlCommand(sorguCumlesi, SqlServerBaglanti.Connection))
				{
		...
		public bool Sil()
		{
			try
			{
				using (var command = new SqlCommand($"DELETE FROM Calisanlar", SqlServerBaglanti.Connection))
				{
					SqlServerBaglanti.BaglantiAc();
					command.ExecuteNonQuery();
				}
		...
		public bool Sil(int id)
		{
			try
			{
				using (var command = new SqlCommand($"DELETE FROM Calisanlar WHERE Id = @p1", SqlServerBaglanti.Connection))
				{
